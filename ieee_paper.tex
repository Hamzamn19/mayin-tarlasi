...
\author{
  \IEEEauthorblockN{Hamzah Sheikh Alashrah$^{1*}$, Yasin Deniz Zeybek$^{2}$, Kemal Gen\c{c}er$^{3}$, Emir Tayan\c{c} Kavak$^{4}$}
  \IEEEauthorblockA{
     Department of Computer Engineering, Beykoz University\\
    Istanbul, Turkey\\
    $^1$hamzahsheikhalashrah@ogrenci.beykoz.edu.tr\\
    $^2$YasinDenizZeybek@ogrenci.beykoz.edu.tr\\
    $^3$KemalGencer@ogrenci.beykoz.edu.tr\\
    $^4$EmirTayancKavak@ogrenci.beykoz.edu.tr
  }
}

\maketitle
\footnotetext{$^*$Corresponding author: Hamzah Sheikh Alashrah (hamzahsheikhalashrah@ogrenci.beykoz.edu.tr)}
